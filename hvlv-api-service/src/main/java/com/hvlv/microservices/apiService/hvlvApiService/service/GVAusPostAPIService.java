...
public class GVAusPostAPIService {

    public static final String CHARGE_CODE_POD = "3D85";
    public static final String CHARGE_CODE_NO_POD = "3J55";
    ...
    static {
        API_ACCOUNTS.put("GVSYD", Map.of(
                "id", "34KHX",
                "no", "0005885071",
                "key", "41e2a10a-1d55-4ce1-894b-e7b97f30559b",
                "password", "mRAoAODlaPfVWTO3lCI0",
                "charge_code_pod", "3D85",
                "charge_code_no_pod", "3J55"
        ));

        API_ACCOUNTS.put("GVPER", Map.of(
                "id", "34KHW",
                "no", "0005885063",
                "key", "41e2a10a-1d55-4ce1-894b-e7b97f30559b",
                "password", "mRAoAODlaPfVWTO3lCI0",
        ...
        API_ACCOUNTS.put("GVMEL2", Map.of(
                "id", "34WMP",
                "no", "0004698477",
                "key", "b440b68a-aaaa-4314-a492-341458856def",
                "password", "rT9kxaUqtwrTb41zp7tR",
                "charge_code_pod", "3D85",
                "charge_code_no_pod", "3J55",
                "org", "Prezario UNO"
        ));
        ...
        API_ACCOUNTS.put("test", Map.of(
                ...
                "charge_code_pod", "7E55",
                "charge_code_no_pod", "7E55"
        ));

        API_ACCOUNTS_INFO.put("syd", Map.of("id", "34KHX"));
        API_ACCOUNTS_INFO.put("mel", Map.of("id", "34KG6"));
        API_ACCOUNTS_INFO.put("bne", Map.of("id", "34KHY"));
        API_ACCOUNTS_INFO.put("per", Map.of("id", "34KHW"));
        API_ACCOUNTS_INFO.put("mel2", Map.of("id", "34WMP"));
    ...
    private String charge_code_pod;
    private String charge_code_no_pod;
    private boolean is_test;
    private boolean debug;

    public GVAusPostAPIService() {
        ...
        if (API_ACCOUNTS.containsKey("test")) {
            ...
            this.charge_code_pod =accountInfo.get("charge_code_pod");
            this.charge_code_no_pod =accountInfo.get("charge_code_no_pod");
        }
    }
    ...
    public Map<String, Double> getVirtualWeightAndDim(double weight) {
        ...
        weight = Math.round(weight * 100) / 100; // 保留两位小数
        width = Math.round(width * 10) / 10; // 保留一位小数
        height = Math.round(height * 10) / 10; // 保留一位小数
        length = Math.round(length * 10) / 10; // 保留一位小数
